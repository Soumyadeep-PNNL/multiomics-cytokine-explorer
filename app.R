...
# Sanitise time_h_clean: keep only values matching \d+h, set rest to NA
sanitize_time_h <- function(x) {
  ifelse(grepl("^\\d+h$", x, perl=TRUE), x, NA_character_)
}
...
for (flt in c("FLAT", "FLAT_BULK", "FLAT_SC")) {
  ...
  if (!is.null(d) && nrow(d) > 0) {
    ...
    d$time_h_clean    <- sanitize_time_h(
                              mapply(derive_time_h, d$time_h, d$comparison,
...
# Same for FLAT_SC — removes BFA, TG, pure NMMA rows; retains all cytokine comparisons
if (!is.null(FLAT_SC) && nrow(FLAT_SC) > 0)
  FLAT_SC <- FLAT_SC[!is.na(FLAT_SC$treatment_clean), , drop=FALSE]
...
make_bulk_dotplot <- function(df, xlabel_fmt="treat_time_model",
  ...
  # One stripe per column: deduplicate on (ds_label, omics_type) so that
  # when two packages share the same ds_label (e.g. Pck001 & Pck011 are both
  # HPI/RNA-seq/IFNg+IL-1b/48h) we don't get phantom extra x_pos values that
  # render as blank columns in the padding area.
  bg <- df %>%
    dplyr::select(ds_label, pkg_id, omics_type) %>%
    dplyr::arrange(omics_type, as.integer(ds_label), pkg_id) %>%
    dplyr::distinct(ds_label, omics_type, .keep_all = TRUE) %>%
  ...
  ggplot(df, aes(x = ds_label, y = gene_name)) +
    ...
    geom_rect(
      data    = bg,
      mapping = aes(xmin  = x_pos - 0.5, xmax = x_pos + 0.48,
    ...
    scale_size_continuous(
      range  = c(1.5, 6),   # max 6mm fits within fixed 0.9in column with padding
      name   = "-log10(p)",
      breaks = c(1, 5, 10, 20),
...
make_sc_dotplot <- function(df) {
  ...
  # ── Build clean 3-line facet label: Model / Treatment / Duration ─────────
  tc <- if ("treatment_clean" %in% names(df) && !all(is.na(df$treatment_clean)))
          df$treatment_clean else df$treatment
  ma <- if ("model_abbrev"   %in% names(df) && !all(is.na(df$model_abbrev)))
          df$model_abbrev   else df$model
  ...
  # ── Significance opacity ─────────────────────────────────────────────────
  has_sig      <- "sig_status" %in% names(df)
  df$pt_alpha  <- if (has_sig) ifelse(df$sig_status == "NS", 0.22, 0.90) else 0.85

  # ── Rich tooltip ─────────────────────────────────────────────────────────
  df$tip_html <- paste0(
    "<b>", df$gene_name, "</b>  [", df$pkg_id, "]<br/>",
    "Cell type: ", df$cell_type, "<br/>",
    "Model: ", ma, " | Treatment: ", tc, " | Time: ", th, "<br/>",
    "log2FC: ", round(df$log2FC, 3),
    ifelse(!is.na(df$padj),
           paste0("  | padj: ", signif(df$padj, 3)),
           ifelse(!is.na(df$pvalue),
  ...
  # ── Package → shape mapping (up to 8 packages) ───────────────────────────
  pkgs       <- sort(unique(as.character(df$pkg_id)))
  shape_vals <- c(16, 17, 15, 18, 8, 10, 11, 12)[seq_along(pkgs)]
  pkg_shapes <- setNames(shape_vals, pkgs)

  n_genes  <- nlevels(df$gene_name)
  n_facets <- nlevels(df$ds_label)

  ggplot(df, aes(x = cell_type, y = gene_name)) +
    # Significant dot outline ring
    geom_point(
      data = df[!is.na(df$sig_status) & df$sig_status != "NS", , drop=FALSE],
      aes(x = cell_type, y = gene_name, size = neglog10p),
      shape = 21, fill = NA, colour = "#444444", stroke = 0.8,
      inherit.aes = FALSE
    ) +
    # Interactive dots — shape encodes package, colour encodes log2FC
    geom_point_interactive(
      aes(size    = neglog10p,
          colour  = log2FC,
          shape   = pkg_id,
          alpha   = I(pt_alpha),
          tooltip = tip_html,
          data_id = paste0(gene_name, "_", cell_type, "_", pkg_id)),
    ) +
    facet_wrap(~ ds_label,
               scales = "free_x",
               ncol   = min(n_facets, 3)) +
    ...
    scale_shape_manual(values = pkg_shapes, name = "Package") +
    scale_size_continuous(
      range  = c(2, 7),
      name   = "-log10(p)",
      breaks = c(1, 5, 10, 20),
    ...
    labs(x = "Cell type", y = NULL,
         caption = if(has_sig) "Opacity: filled = significant, faded = NS" else NULL) +
    theme_bw(base_size = 12) +
    theme(
      axis.text.x      = element_text(angle = 45, hjust = 1, size = 9),
      axis.text.y      = element_text(size = 10),
      strip.text       = element_text(face = "bold", size = 10,
                                      color = "#2c3e50", lineheight = 0.9),
      strip.background = element_rect(fill = "#dde3ea", color = "#b0bec5"),
...
# ===========================================================================
ui <- page_navbar(
  title = "T1D in vitro Multiomics Data Explorer",
  ...
  nav_panel("Overview",
    layout_sidebar(
      sidebar = sidebar(width=280,
        h5("About"),
        tags$p(style="font-size:12px;line-height:1.5;",
          "A dataset resource from previously published multi-omics studies of",
          " pancreatic islets and β-cell models exposed to pro-inflammatory cytokines.",
          " 70 datasets from 23 studies, organized into 25 data packages",
          " (Pck001–Pck025) via PNNL DataHub."),
        tags$p(style="font-size:12px;line-height:1.5;margin-top:6px;",
          tags$b("Models: "),
          "Primary human islets, EndoC-βH1, iPSC-derived islet-like cells,",
          " mouse islets, MIN6, β-TC-3, β-TC-6."),
        tags$p(style="font-size:12px;line-height:1.5;margin-top:4px;",
          tags$b("Cytokines: "),
          "IFNα, IFNγ, IL-1β, TNFα and combinations."),
        hr(),
  ...
  nav_panel("Single Cell",
    layout_sidebar(
      sidebar = sidebar(width=300,
        ...
        selectizeInput("gsc_treat","Treatment",         choices=NULL, multiple=TRUE),
        selectizeInput("gsc_time", "Duration",          choices=NULL, multiple=TRUE),
      ...
      navset_tab(
        nav_panel("Dot Plot",
          div(style="overflow:auto; width:100%; border:1px solid #e0e0e0; border-radius:4px;",
            girafeOutput("gsc_dotplot", height="auto", width="100%"))),
...
server <- function(input, output, session) {
  ...
  output$gs_dotplot <- renderGirafe({
    ...
    # Fixed 0.90 in per column: wide enough for max dot (6mm) + 3-line label at 90deg
    # Container scrolls horizontally; all columns are identical width
    w_svg <- max(8, n_cols * 0.90)
  ...
  # ── Single Cell: cascading filters ───────────────────────────
  observe({
    models <- sort(unique(na.omit(FLAT_SC$model_abbrev)))
  ...
  observe({
    ...
    if (length(sel_m) > 0) df <- df[df$model_abbrev %in% sel_m, ]
    treats <- sort(unique(na.omit(df$treatment_clean)))
    updateSelectizeInput(session,"gsc_treat", choices=c("(all treatments)"="", treats), server=TRUE)
  })

  observe({
    df    <- FLAT_SC
    sel_m <- input$gsc_model[nzchar(input$gsc_model)]
    sel_t <- input$gsc_treat[nzchar(input$gsc_treat)]
    if (length(sel_m) > 0) df <- df[df$model_abbrev    %in% sel_m, ]
    if (length(sel_t) > 0) df <- df[df$treatment_clean %in% sel_t, ]
    times <- sort(unique(na.omit(df$time_h_clean)))
    updateSelectizeInput(session,"gsc_time", choices=c("(all durations)"="", times), server=TRUE)
  })
  ...
  observe({
    ...
    sel_m <- input$gsc_model[nzchar(input$gsc_model)]
    sel_t <- input$gsc_treat[nzchar(input$gsc_treat)]
    sel_h <- input$gsc_time[ nzchar(input$gsc_time)]
    if (length(sel_m) > 0) df <- df[df$model_abbrev    %in% sel_m, ]
    if (length(sel_t) > 0) df <- df[df$treatment_clean %in% sel_t, ]
    if (length(sel_h) > 0) df <- df[df$time_h_clean    %in% sel_h, ]
    cts <- sort(unique(na.omit(df$cell_type)))
  ...
  gsc_all <- eventReactive(input$gsc_search, {
    ...
    if (length(input$gsc_ct) > 0)
      df <- df[df$cell_type %in% input$gsc_ct, ]
  ...
  output$gsc_dotplot <- renderGirafe({
    res <- gsc_all(); req(!is.null(res))
    df  <- res$data
    if (is.null(df) || nrow(df) == 0) {
      p <- ggplot() + labs(title="No scRNA results found") + theme_minimal()
      return(girafe(ggobj=p, width_svg=8, height_svg=4))
    }
    p        <- make_sc_dotplot(df)
    n_genes  <- length(unique(df$gene_name))
    n_ct     <- length(unique(df$cell_type))
    n_facets <- length(unique(paste0(df$model_abbrev, df$treatment_clean, df$time_h_clean)))
    n_cols   <- min(n_facets, 3)
    n_rows   <- ceiling(n_facets / n_cols)
    w_svg    <- max(8, n_cols * max(3, n_ct * 0.55))
    h_svg    <- max(5, n_rows * (n_genes * 0.45 + 2.5))
    girafe(
      ggobj      = p,
      width_svg  = w_svg,
      height_svg = h_svg,
      options = list(
        opts_tooltip(
          css = paste0("background:white;border:1px solid #bbb;",
                       ...
                       "font-size:12px;line-height:1.5;",
                       "box-shadow:2px 2px 6px rgba(0,0,0,.15);"),
          use_fill = FALSE, delay_mouseover = 80
        ),
        opts_hover(css = "stroke:#333;stroke-width:1.2;cursor:pointer;"),
        opts_zoom(min=0.5, max=5),
        opts_sizing(rescale=FALSE)
